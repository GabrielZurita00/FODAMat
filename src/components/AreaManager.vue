    ...
    <div class="main-div">
      <div class="logo">
        ...
        <a style="color:#000000;" href="/">
          <div class="area-name add-area">
            <svg fill="#000000" height="20" width="20" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
      ...
      <div class="area-manager">
        <div class="vertical-menu" v-if="count >= 0">
          ...
          <div class="area-name area-selection" v-for="area in count" :key="area" @click="showArea(area)"
            v-bind:class="((area != selected) && (!totalArea)) ? 'unselected-area' : 'selected-area'">
            <h2>Área {{ area }}</h2>
      ...
      <div class="area-info">
        <h1>Tabla FODA</h1>
        <button style="color:#000000;" @click="print">
          <div class="area-name add-area">
            <svg fill="#000000" height="20" width="20" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 26.676 26.676" xml:space="preserve">
              <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
      ...
      <div class="foda-manager" v-if="!totalArea">
        <table>
          <tr>
            ...
            <th :colspan="values.o + values.a + 1">
              <h3>ÁMBITO EXTERNO</h3>
            </th>
          </tr>
          <tr>
            <td style="border:0"></td>
          ...
          <tr>
            <th :rowspan="values.f + values.d + 2" width="15">
              <h3 style="transform: rotate(-90deg); text-wrap: wrap;">ÁMBITO INTERNO</h3>
            </th>
          </tr>
          <tr v-for="(row, rowIndex) in values.f" :key="rowIndex">
            ...
            <td v-for="(col, colIndex) in values.o" :key="colIndex">
              <dropdown :options="dropdownOptions" :matrixValue="values.matriz[rowIndex][colIndex]"
                @option-selected="onOptionSelected(rowIndex, colIndex, $event)" />
            </td>
            <td v-for="(col, colIndex) in values.a" :key="colIndex">
              <dropdown :options="dropdownOptions" :matrixValue="values.matriz[rowIndex][colIndex + values.o]"
                @option-selected="onOptionSelected(rowIndex, colIndex + values.o, $event)" />
            </td>
          ...
          <tr v-for="(row, rowIndex) in values.d" :key="rowIndex">
            ...
            <td v-for="(col, colIndex) in values.o" :key="colIndex">
              <dropdown :options="dropdownOptions" :matrixValue="values.matriz[rowIndex + values.f][colIndex]"
                @option-selected="onOptionSelected(rowIndex + values.f, colIndex, $event)" />
            </td>
            <td v-for="(col, colIndex) in values.a" :key="colIndex">
              <dropdown :options="dropdownOptions" :matrixValue="values.matriz[rowIndex + values.f][colIndex + values.o]"
                @option-selected="onOptionSelected(rowIndex + values.f, colIndex + values.o, $event)" />
            </td>
      ...
      <div class="foda-manager" v-if="totalArea">
        ...
        <h1 class="graph-title">Análisis FODA</h1>
        <div class="bar-graph" id="radarchart">
      ...
      <div id="fulltable">
        ...
        <div class="foda-manager">
          ...
          <h1 class="graph-title">Análisis FODA</h1>
          <div class="bar-graph" id="imgchart">
          </div>
        </div>
      </div>
    </div>
...
export default {
  components: { Dropdown, PieGraph, BarGraph, RadarChart, jsPDF, html2canvas },
  name: "AreaManager",
  watch: {
